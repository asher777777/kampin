...
async function run() {
  const convsSnap = await db.collection("employees").doc("walker").collection("conversations")
    .orderBy("lastUpdatedAt", "desc").limit(1).get();
  
  if (convsSnap.empty) {
    console.log("No conversations found for Walker.");
    return;
  }
  
  const doc = convsSnap.docs[0];
