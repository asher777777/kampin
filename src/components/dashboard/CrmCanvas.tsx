...
import { Send, Bot, User, Loader2, Users, Plus, List, FileDown, FileUp, UserPlus, Trash2 } from "lucide-react";
...
export function CrmCanvas() {
  ...
  const initialContext = "dashboard/crm";

  // Load History
  useEffect(() => {
    async function fetchHistory() {
      const res = await loadChatHistory(`/${initialContext}`);
  ...
  const handleSend = async (text: string = input) => {
    ...
    const res = await chatWithAssistant(sessionId, text, `/${initialContext}`);
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50/50 w-full relative" dir="rtl">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-8 space-y-6">
        ...
        {/* Initial Prompt */}
        {showInitialPrompt && (
          <div className="flex flex-col items-center justify-center py-6 max-w-2xl mx-auto text-center space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="w-16 h-16 bg-[#0f172a] rounded-full flex items-center justify-center shadow-lg border border-white/10 shrink-0">
              <Bot className="w-10 h-10 text-indigo-400" />
            </div>
            <div className="space-y-2">
              <h2 className="text-2xl font-black text-slate-800 tracking-tight">איך מחזקים את הקשרים?</h2>
              <p className="text-slate-600 text-lg">
                בחר פעולה מהירה או חפש משהו ספציפי
              </p>
            </div>
            
            <div className="flex flex-col w-full mt-6 max-w-sm mx-auto gap-3 px-4">
              <button
                onClick={() => handleSend("הצג את 10 הקונים האחרונים")}
                className="flex items-center gap-3 px-5 py-4 bg-white border border-slate-200/80 rounded-2xl hover:border-indigo-400 hover:shadow-md shadow-sm transition-all group"
              >
                <div className="p-2 bg-blue-50 text-blue-600 rounded-xl group-hover:scale-110 transition-transform">
                  <Users className="w-5 h-5" />
                </div>
                <span className="font-bold text-slate-700">10 הקונים האחרונים</span>
              </button>

              <button
                onClick={() => handleSend("עם מי לא דיברתי בחודש האחרון?")}
                className="flex items-center gap-3 px-5 py-4 bg-white border border-slate-200/80 rounded-2xl hover:border-indigo-400 hover:shadow-md shadow-sm transition-all group"
              >
                <div className="p-2 bg-amber-50 text-amber-600 rounded-xl group-hover:scale-110 transition-transform">
                  <Users className="w-5 h-5" />
                </div>
                <span className="font-bold text-slate-700">עם מי לא דיברתי בחודש האחרון?</span>
              </button>

              <button
                onClick={() => handleSend("הצג לידים שלא קיבלו התייחסות")}
                className="flex items-center gap-3 px-5 py-4 bg-white border border-slate-200/80 rounded-2xl hover:border-indigo-400 hover:shadow-md shadow-sm transition-all group"
              >
                <div className="p-2 bg-rose-50 text-rose-600 rounded-xl group-hover:scale-110 transition-transform">
                  <Users className="w-5 h-5" />
                </div>
                <span className="font-bold text-slate-700">הלידים שלא קיבלו התייחסות</span>
              </button>
            </div>
          </div>
        )}
                ...
                <div className="prose prose-sm md:prose-base prose-slate rtl text-right max-w-none prose-p:leading-relaxed prose-pre:bg-slate-800 prose-pre:text-slate-100">
                  <ReactMarkdown 
                    components={{
                      ...
                      a: ({ node, ...props }) => {
                        if (props.href?.startsWith("#chat-action:")) {
                          return (
                            <button
                              onClick={(e) => {
                                e.preventDefault();
                                const action = props.href?.replace("#chat-action:", "");
                                if (action) handleSend(action);
                              }}
                              className="inline-flex items-center gap-1 bg-blue-50/80 px-2.5 py-1 rounded-md text-blue-700 hover:bg-blue-100 transition-colors font-semibold border border-blue-200/50 my-0.5"
                            >
                              {props.children}
                            </button>
                          );
