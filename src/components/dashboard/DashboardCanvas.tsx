...
import { Send, Bot, User, Loader2, FileText, Sparkles, Megaphone, Trash2 } from "lucide-react";
...
export function DashboardCanvas({ pagesCount, initialContext = "dashboard" }: DashboardCanvasProps) {
        ...
        {showInitialPrompt && (
          <div className="flex flex-col items-center justify-center min-h-[50vh] max-w-2xl mx-auto text-center space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="space-y-2">
              <h2 className="text-2xl font-black text-slate-800 tracking-tight">שלום! שמח לראות אותך כאן.</h2>
              <p className="text-slate-600 text-lg">
                {pagesCount < 5 
                  ? "אני רואה שעדיין אין לך מספיק תוכן במערכת. מה תרצה ליצור היום?"
                  : "מה תרצה לעשות היום עם הקהילה שלך?"}
              </p>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full mt-8">
              <button
                onClick={() => handleSend("אני רוצה ליצור עמוד נחיתה חדש")}
              ...
              >
                ...
                <span className="font-bold text-slate-800">עמוד נחיתה</span>
                <span className="text-xs text-slate-500 text-center">לאיסוף לידים והרשמות</span>
              </button>

              <button
                onClick={() => handleSend("אני רוצה ליצור עמוד שירות חדש")}
                className="flex flex-col items-center p-6 bg-white border border-slate-200 rounded-2xl hover:border-indigo-500 hover:shadow-md hover:bg-indigo-50/30 transition-all gap-3 group"
              >
                <div className="p-3 bg-emerald-50 text-emerald-600 rounded-xl group-hover:scale-110 transition-transform">
                  <Sparkles className="w-6 h-6" />
                </div>
                <span className="font-bold text-slate-800">עמוד שירות</span>
                <span className="text-xs text-slate-500 text-center">הצגת השירותים והמוצרים שלך</span>
              </button>

              <button
                onClick={() => handleSend("אני רוצה ליצור עמוד תוכן עבור SEO")}
                className="flex flex-col items-center p-6 bg-white border border-slate-200 rounded-2xl hover:border-indigo-500 hover:shadow-md hover:bg-indigo-50/30 transition-all gap-3 group"
              >
                <div className="p-3 bg-purple-50 text-purple-600 rounded-xl group-hover:scale-110 transition-transform">
                  <FileText className="w-6 h-6" />
                </div>
                <span className="font-bold text-slate-800">תוכן ו-SEO</span>
                <span className="text-xs text-slate-500 text-center">מאמרים וקידום אורגני</span>
              </button>
            </div>
          </div>
        )}
