...
export const LiveDonationAlert: React.FC<LiveDonationAlertProps> = ({
  ...
  // 1. Initial Mount: Load the 3 most recent donations and queue them
  useEffect(() => {
    let isMounted = true;
    ...
    const loadInitialDonors = async () => {
      try {
        ...
        // Queue all 3 items to pop up sequentially starting after 1.2s
        setTimeout(() => {
          if (isMounted) {
            setQueue((prev) => {
  ...
  }, [targetCampaignId, recentDonations]);
  ...
  // 3. Countdown timer and auto-dismiss
  useEffect(() => {
  ...
  // Position classes (RTL compliant with high z-index)
  const positionClasses = {
    "bottom-right": "bottom-24 sm:bottom-8 right-4 sm:right-8",
    "bottom-left": "bottom-24 sm:bottom-8 left-4 sm:left-8",
    "top-right": "top-20 right-4 sm:right-8",
  ...
  return (
    <div 
      className={`fixed z-[9999] pointer-events-none ${positionClasses} max-w-[calc(100vw-2rem)] sm:max-w-[360px] w-full`}
      dir="rtl"
      ...
      {enableDemoMode && (
        <div className="pointer-events-auto mb-2 flex justify-end">
          <button
            ...
            className="text-xs bg-amber-500/20 text-amber-300 hover:bg-amber-500/30 border border-amber-500/40 px-2.5 py-1 rounded-full flex items-center gap-1.5 shadow-sm transition-colors cursor-pointer"
          >
            <Sparkles className="w-3 h-3 text-amber-400" />
            <span>בדיקת כרטיס תרומה</span>
          </button>
        </div>
      )}
      ...
      <AnimatePresence mode="wait">
        {currentDonation && (
          <motion.div
            ...
            className={`pointer-events-auto relative overflow-hidden rounded-2xl bg-white/95 text-slate-900 p-3.5 sm:p-4 shadow-2xl border border-amber-300/80 backdrop-blur-md cursor-pointer group transition-all duration-300 hover:border-amber-400 hover:shadow-amber-500/20`}
            style={{
              boxShadow: "0 12px 36px -4px rgba(0, 0, 0, 0.1), 0 0 20px 2px rgba(245, 158, 11, 0.12)",
            }}
          >
            {/* Top Ambient Campaign Glows */}
            <div className="absolute -top-10 -right-10 w-24 h-24 bg-emerald-400/10 rounded-full blur-2xl pointer-events-none" />
            <div className="absolute -bottom-8 -left-8 w-24 h-24 bg-amber-300/15 rounded-full blur-xl pointer-events-none" />

            {/* Countdown Progress Bar */}
            <div className="absolute top-0 right-0 left-0 h-[3px] bg-amber-100/70 overflow-hidden">
              <motion.div
            ...
            <button
              ...
              className="absolute top-2.5 left-2.5 text-slate-400 hover:text-slate-700 p-1 rounded-full hover:bg-slate-100 transition-colors z-20 cursor-pointer"
            >
              <X className="w-3.5 h-3.5" />
            </button>

            {/* Main Content Layout: Tier Button Image + Donor Name + Dedication ONLY */}
            <div className="relative z-10 flex items-center gap-3.5">
              
              {/* 1. Tier / Button Image in Campaign Colors */}
              <div className="relative shrink-0">
                <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-gradient-to-br from-amber-400 via-emerald-500 to-amber-500 p-[2px] shadow-md shadow-emerald-950/5">
                  <div className="w-full h-full bg-white rounded-[14px] overflow-hidden flex items-center justify-center">
                    {tierInfo.imageSrc ? (
                      ...
                      <div className="flex flex-col items-center justify-center text-amber-500 text-center p-1">
                        <Heart className="w-5 h-5 fill-amber-500 text-amber-500 animate-pulse" />
                        <span className="text-[10px] font-black text-slate-800 mt-0.5 leading-none">
                          {tierInfo.title || "תרומה"}
                        </span>
                      </div>
                    )}
                  </div>
              ...
              {/* 2. & 3. Donor Name & Dedication ONLY */}
              <div className="flex-1 min-w-0 pr-0.5 space-y-1">
                
                {/* Donor Name */}
                <h4 className="font-black text-base sm:text-lg text-slate-900 truncate tracking-tight">
                ...
                {/* Dedication */}
                <p className="text-xs sm:text-sm text-slate-800 italic line-clamp-2 leading-relaxed bg-amber-50/90 px-2.5 py-1.5 rounded-xl border border-amber-200/80 font-medium">
                  &quot;{currentDonation.dedication || "תרומה מכל הלב לחיזוק והצלחת הקמפיין! ❤️"}&quot;
                </p>
