...
import { Wand2, Sparkles, ChevronDown, Eye, Send, Bot, User, CheckCircle2, ArrowLeft } from "lucide-react";

const DEFAULT_FORM: FormConfig = {
  enabled: true,
  form_type: "standard",
  submit_button_text: "שלח",
  submit_button_bg_color: "#f59e0b",
  submit_button_text_color: "#000000",
...
export default function TestAIFormPage() {
  ...
  // Initialize first message with user name when component mounts or user loads
  useEffect(() => {
  ...
  const handleSend = async (textOverride?: string) => {
    ...
    // All onboarding done — build the full prompt and generate
...
מטרת הטופס: ${newCollected["goal"]}
  ...
  return (
    <div className="min-h-screen bg-[#0e0e10] text-white" dir="rtl">
      <Navbar />

      {/* Fullscreen loading (Stardust theme) */}
      {isGenerating && (
        <div className="fixed inset-0 z-50 bg-[#000000]/95 backdrop-blur-sm flex flex-col items-center justify-center gap-8 overflow-hidden">
          {/* Stardust particles */}
      ...
      <div className="max-w-3xl mx-auto p-4 md:p-8 pt-28 space-y-2">
          ...
          >
            <span className="flex items-center gap-3">
              ...
              <span className="text-amber-50">
                מיכאל המחלל
                {onboardingStep === "audience" && " - אני כאן אם אתם צריכים עזרה אנטלגנטית"}
              </span>
              {messages.length > 1 && (
          ...
          {aiOpen && (
            <div className="border-t border-amber-500/10 flex flex-col" style={{ height: "480px" }}>
              ...
              <div ref={chatScrollRef} className="flex-1 overflow-y-auto p-4 space-y-5 bg-[#0a0a0c] relative">
                {/* Subtle background stardust effect for chat area */}
              ...
              {/* Quick option buttons (Pinned above input) */}
        ...
        {/* תצוגה מקדימה */}
        <div className="w-full bg-[#181818] border border-white/5 rounded-2xl overflow-hidden">
          <button
            type="button"
          ...
          >
            ...
            <ChevronDown className={`w-4 h-4 transition-transform text-gray-400 ${previewOpen ? "rotate-180 text-white" : ""}`} />
          </button>

          {previewOpen && (
            <div className="p-6 bg-[#111] border-t border-white/5">
              <div className="bg-black/40 rounded-2xl overflow-hidden flex items-center justify-center p-8 min-h-[300px] border border-amber-500/20 shadow-inner">
                <div className="w-full max-w-md bg-[#111111] rounded-3xl p-6 shadow-[0_0_40px_rgba(245,158,11,0.1)] border border-amber-500/20">
